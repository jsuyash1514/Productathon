...
firebase_admin.initialize_app(cred, {
    'databaseURL': 'https://gharkikheti-5f2df.firebaseio.com/'
})
...
class Bot(object):
    ...
    def bot_options(self, action):
        ...
        try:
            ...
            if len(command_args) == 1 and command_args[0] in simple_menu_keys:
                ...
                elif command_args[0] == "show_feed":
                    send_message("Fetching feed...")
                    ref = db.reference('feeds1')
                    snapshot = ref.order_by_child('timestamp').limit_to_first(3).get()
                    send_message("Feed follows:")
                    for key in snapshot:
                        send_message(snapshot[key]['video_description']+"\n" + snapshot[key]['videoUrl'])
                    
                elif command_args[0] == "show_schemes":
                    send_message("Fetching schemes...")
                    ref = db.reference('schemes')
                    snapshot = ref.order_by_child('timestamp').limit_to_first(3).get()
                    send_message("Schemes follows:")
                    for key in snapshot:
                        send_message(snapshot[key]['description'])
    ...
    def _help_commands():
        ...
        return "List of commands:\n" \
               ...
               "/show_feed (get community feed), " \
               "/show_schemes (show latest schemes), " \
...
class GoogleResults(object):        # Make this parent
    ...
    def _attach_and_send_screenshot(self):
        ...
        send_file_xpath = '//*[@id="app"]/div/div/div[2]/div[2]/span/div/span/div/div/div[2]/span/div/div/span'
